...
const CACHE=`${CACHE_PREFIX}v8-private-vary-range-safe-shell`;
...
const hasUnsafeVary=response=>(response.headers.get('vary')||'').split(',').some(value=>['*','cookie','authorization','range'].includes(value.trim().toLowerCase()));
