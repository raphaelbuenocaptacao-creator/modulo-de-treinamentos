...
const CACHE=`${CACHE_PREFIX}v6-isolated-private-safe-shell`;
...
const hasSensitiveVary=response=>(response.headers.get('vary')||'').split(',').some(value=>['cookie','authorization'].includes(value.trim().toLowerCase()));
